...
Claude 호출 순서:
1. `search_bills(assembly="22", bill_name="인공지능", page_size=50)` → 59건 발견
...
Claude 호출 순서:
1. `get_bill_review(assembly="22", bill_no="2206772")` → 위원회 + 본회의 타임라인
...
Claude 호출 순서:
1. `get_vote_results(assembly="22", bill_name="법원조직법")` → 의안 확인 + 집계 + BILL_ID 획득
2. `get_member_votes(bill_id="PRC_H2W6O0K2D1T1Y2B0O5J2K5Q5A8Z0Y3", assembly="22")` → 의원별 표결 295행
...
Claude 호출 순서:
1. `get_member_info(assembly="22", name="이준석")` → 정당, 지역구, 위원회
2. `search_bills(assembly="22", proposer="이준석", page_size=100)` → 전체 발의 법안
3. `get_vote_results(assembly="22", page_size=50)` → 최근 표결 법안 목록
4. 각 법안: `get_member_votes(bill_id=..., assembly="22", member_name="이준석")`
...
Claude 호출:
1. `get_pending_bills(assembly="22", committee="과학기술정보방송통신위원회", page_size=100)` → 12,505건
2. `get_pending_bills(assembly="22", committee="과학기술정보방송통신위원회", bill_name="인공지능")` → 31건
...
Claude 호출:
1. `get_plenary_agenda(assembly="22", page_size=30)` → 상정 예정 안건
...
- `search_bills`에 클라이언트 사이드 날짜 필터링 추가 (`propose_dt_from`/`propose_dt_to`)
- `get_member_info`를 ALLNAMEMBER 엔드포인트 기반으로 수정 (대수별 정확한 정당/선거구/위원회 데이터)
