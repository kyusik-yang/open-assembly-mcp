...
**데이터 범위**: 16~22대 국회 (2000년~현재). 의원 발의안 전용 (정부제출안 제외).

| 도구 | 설명 |
|---|---|
| `search_bills` | 대수, 키워드, 발의자, 위원회, 기간, 처리결과로 법률안 검색 |
| `get_bill_detail` | 의안 상세정보 (처리 이력, 본회의 결과 포함) |
| `get_member_info` | 의원 프로필 (정당, 선거구, 위원회, 선수) |
| `get_vote_results` | 의안별 본회의 표결 집계 (찬성/반대/기권 건수) |
| `get_member_votes` | 특정 법안에 대한 의원 개인별 표결 기록 |
| `get_bill_proposers` | 법안의 대표발의자 및 공동발의자 목록 |
| `get_bill_review` | 위원회~본회의 법안 심사 타임라인 |
| `get_committee_members` | 위원회 위원 명단 |
